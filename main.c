...
static void *ai_feed_thread(void *arg) {
    (void)arg;
    while (running) {
        queued_frame_t f; if (queue_get(&g_queue,&f)!=0) break;
        ...
        pthread_mutex_unlock(&g_anno_lock);
        if(!pushed) gst_pipeline_push_frame(f.data,f.size,f.frame_id);
        ...
        cv_branch_push_frame(&cvf); free(f.data);
    }
    return NULL;
}
